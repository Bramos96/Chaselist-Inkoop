...
def build_mail_body(lang: str, name: str, html_table: str, nl_tmpl: str, en_tmpl: str) -> str:
    ...
    # 2️⃣ Collapse extra blank lines (Outlook-safe)
    body = re.sub(r"(<br>\s*){3,}", "<br><br>", body)     # no triple spacing
    body = re.sub(r"(<br>\s*)+$", "", body)               # trim trailing breaks

    # 3️⃣ Ensure exactly ONE blank line before table, NONE after text
...
def main():
    ...
    for supplier, grp in df.groupby(supplier_col):
        try:
            ...
            subj = (
                "Pending order(s) - VHE Industrial Automation"
                if lang == "ENG"
                else "Openstaande bestelling(en) - VHE Industrial Automation"
            )

            to_addr = TEST_EMAIL if TESTMODE else TEST_EMAIL
